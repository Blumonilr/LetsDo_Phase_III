...
function uploadTest(){
    $.ajax({
        url: "/publisherPage/publishTestPage",
        type: "post",
        dataType:"text",
        data:{"projectId":getCookie("projectId")},
        success : function(data){
            if(data==="alreadyHaveTest"){
                alert("已经上传过测试集");
            }else{
                window.location.href="/publisherPage/publishTestPage"
            }
        },
        error : function(XMLHttpRequest, textStatus, errorThrown) {
            alert(XMLHttpRequest+"///"+textStatus+"///"+errorThrown);
...
$(function () {
    ...
    if(projectId===""){
        alert("请先选择项目");
        window.location.href="/project/publisherProjects";
        return;
    }
